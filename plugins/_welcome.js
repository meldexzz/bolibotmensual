...
export async function before(m, { conn, participants, groupMetadata }) {
  ...
  if (!img) {
    img = getGroupIcon(m); // Si no hay imagen de perfil, intenta usar la del grupo
  }
    ...
    switch (m.messageStubType) {
      case WAMessageStubType.GROUP_PARTICIPANT_ADD:
        message = chat.sWelcome
          ...
          : `⭐ _Hola *${taguser}*, bienvenid@ al grupo *${groupMetadata.subject}*._`;
        break;
      case WAMessageStubType.GROUP_PARTICIPANT_REMOVE:
        message = chat.sBye
          ? chat.sBye.replace('@user', taguser).replace('@subject', groupMetadata.subject)
          : `☠️ _*${taguser}* fue expulsad@ del grupo._`;
        break;
