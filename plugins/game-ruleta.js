...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  if (limit > 50) return conn.reply(m.chat, "🚩 La cantidad máxima de apuesta es de 50 *❇️ Eris*.", m, rcanal)

  if (!(color === 'black' || color === 'red')) return conn.reply(m.chat, "🚩 Debes apostar a un color válido: *black* o *red*.", m, rcanal)

  if (limit > users.limit) return conn.reply(m.chat, "🚩 No tienes suficientes *❇️ Eris* para realizar esa apuesta.", m, rcanal)

  await conn.reply(m.chat, `🚩 Apostaste ${limit} *❇️ Eris* al color ${color}. Espera *⏱ 10 segundos* para conocer el resultado.`, m, rcanal)
  ...
  setTimeout(() => {
    ...
    if (win) {
      users.limit += limit
      conn.reply(m.chat, `🚩 ¡Ganaste! Obtuviste ${limit} *❇️ Eris*. Total: ${users.limit} *❇️ Eris*.`, m, rcanal)
    } else {
      users.limit -= limit
      conn.reply(m.chat, `🚩 Perdiste. Se restaron ${limit} *❇️ Eris*. Total: ${users.limit} *❇️ Eris*.`, m, rcanal)
    }
